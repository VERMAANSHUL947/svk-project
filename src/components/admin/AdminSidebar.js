...
export default function AdminSidebar() {
    ...
    const handleLogout = async () => {
        try {
            ...
            // Use router.push then window location to ensure state clear
            window.location.href = '/admin';
        } catch (error) {
            console.error('Logout failed', error);
            // Fallback clear even if API fails
            localStorage.removeItem('adminUser');
            window.location.href = '/admin';
        }
    };
    ...
    return (
        <aside className="sidebar">
            ...
            <nav className="nav-section">
                ...
                <Link href="/admin/partners" className={`nav-item ${isActive('/admin/partners') ? 'active' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>
                    Partners
                </Link>
                <Link href="/admin/financials" className={`nav-item ${isActive('/admin/financials') ? 'active' : ''}`}>
                ...
                <Link href="/admin/bookings" className={`nav-item ${isActive('/admin/bookings') ? 'active' : ''}`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                    Bookings
                </Link>
                <Link href="/admin/featured-services" className={`nav-item ${isActive('/admin/featured-services') ? 'active' : ''}`}>
